...
create_portal(){
    ...
    /usr/bin/sudo -u nginx /bin/bash -c "/var/www/applications/dashboard/current/bin/app-incubator sync-tenants "
    /usr/bin/sudo -u nginx /bin/bash -c "/var/www/applications/dashboard/current/bin/app-incubator fix-admins-tenants "

    /usr/bin/sudo -u nginx /bin/bash -c "/var/www/applications/dashboard/current/bin/app-incubator sync-users "
}
...
insert_license() {
    ...
    pass=$(/usr/bin/sudo -u nginx dialog --title "Insert License" --backtitle "$backtitle" --editbox /var/www/applications/dashboard/incubator_dashboard.new 16 51 3>&1 1>&2 2>&3)
    exit=$?
    if [ ! ${exit} = 0 ]; then
        return
    fi
...
fetch_data() {
    start=$(dialog --title "Starting date" --backtitle "$backtitle" --date-format '%Y%m%d' --calendar "Please choose the date where we start gather data" 16 51 `date -d '-1 month' +"%d %m %Y"` 3>&1 1>&2 2>&3)
    echo "Start: ${start}"
    exit=$?
    if [ ! ${exit} = 0 ]; then
        return
    ...
    end=$(dialog --title "Ending date" --backtitle "$backtitle" --date-format '%Y%m%d' --calendar "Please choose the date where we finish gather data" 16 51 `date -d '-1 day' +"%d %m %Y"` 3>&1 1>&2 2>&3)
    echo "End: ${end}"
    exit=$?
    if [ ! ${exit} = 0 ]; then
        return
...
OPTION=$(whiptail --cancel-button "Quit" --title "QuiVIDEO Main Menu" --backtitle "$backtitle" --menu "Select which action you intend to do" 30 60 6 "1" "Set machine IP" "2" "Restart web services" "3" "Set dashboard server name" "4" "Add Portal to dashboard" "5" "Show Dashboard License" "6" "Fetch Data"  3>&1 1>&2 2>&3)
